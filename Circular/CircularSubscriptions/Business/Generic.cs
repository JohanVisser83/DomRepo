...
namespace CircularSubscriptions.Business
{
    public class Generic : IGeneric
    {
        ...
        public async Task<APIResponse> SendOTP(string UserName, bool loginflow)
        {
            ...
            {
                var sentOTPMail = await _customerService.SendOTPMailForSubscriptionPortal(UserName, lstOTP.ToString());
            }
            return objResponse;
        }

        public async Task<APIResponse> SendOTPOnMobile(string UserName, bool loginflow)
        {
            var lstOTP = _helper.GenerateRandomNumber(int.Parse(_configuration["OTP:Length"]), _configuration["OTP:MasterOTP"],
                bool.Parse(_configuration["OTP:IsMasterOTPEnabled"]), bool.Parse(_configuration["OTP:IsAlphaNumeric"]));
            var response = await SaveOTPAsync(UserName, lstOTP.ToString(), loginflow);
        ...
        public async Task<APIResponse> verifyOTP(string userName, string otp)
        {
